...
function registerHookProvider(){
    ...
    var hookMethod = function (hookMethod, mycallback) {
    ...
    var unhookMethod = function (hookMethod, additionalObj) {
    ...
    var provider
    try{
        provider = XServerFactory.registerClass({
            ...
            methods: {
                ...
                unhookMethod: [{
                    ...
                    implementation: unhookMethod
                }]
            }
        });
    }catch(e){
        console.log("Ops, registerClass Failed, try DummyProvider way")
        provider = XServerFactory.use("monkeylord.XServer.handler.Hook.DummyProvider");
        provider.hookMethod.implementation = hookMethod
        provider.unhookMethod.implementation = unhookMethod
    }
...
function loadXServer(path){
    ...
    var cacheDir = "/data/data/" + app.getPackageName() + "/cache"
    var codeCacheDir = "/data/data/" + app.getPackageName() + "/code_cache"
    var SystemProperties = Java.use("android.os.SystemProperties")
    if(SystemProperties.getInt("ro.build.version.sdk", 0)>=21){
        codeCacheDir = app.getCodeCacheDir().toString()
        cacheDir = app.getCacheDir().toString()
    }
    
    Java.classFactory.cacheDir = cacheDir
    Java.classFactory.codeCacheDir = codeCacheDir
    ...
    XServerFactory.cacheDir = cacheDir
    XServerFactory.codeCacheDir = codeCacheDir
